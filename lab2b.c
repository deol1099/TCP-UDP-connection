...
void writeDataFrame(FRAME f,int flag) {
    ...
    if(conn[flag].link!=-1)
        ...
        {
            CHECK(CNET_write_physical(i, &f, &length));
        }
    }
...
//  CALCULATE THE CHECKSUM OF THE ARRIVING FRAME, IGNORE IF INVALID
    ...
    frame.checksum = x;
    frame.hops = frame_hops + 1;
    if(frame.destaddr!=nodeinfo.address) {
        for(int i=1;i<=nodeinfo.nlinks;i++){
            if(link!= i)
                CHECK(CNET_write_physical(i, &frame, &len));
        }
        return;
    }
...
//  AN ACKNOWLEDGMENT HAS ARRIVED, ENSURE IT'S THE ACKNOWLEDGMENT WE WANT
    ...
    if(frame.ack != -1 && frame.ack == conn[flag].ackexpected) {
            printf("\t\t\t\t ACK received,src = %d, dest=%d, seq=%i, ack=%d, msglen=%lu, expectedhops=%d, hops=%d\n", frame.srcaddr, frame.destaddr, frame.seq, frame.ack, frame.len, conn[flag].expectedHops, frame.hops);
            CNET_stop_timer(conn[flag].lasttimer);
            if(conn[flag].link==-1){ 
                conn[flag].expectedHops=frame.hops;
                conn[flag].link = link; // set the link number
            }
            if(frame.hops!=conn[flag].expectedHops){
                printf("IGNORED\n");
                return; //No acknowledgement sent
            }
...
static EVENT_HANDLER(showstate) {
    for(int i=0;i<numOpenConn;i++) {
        printf("Connection =%d,\n\tackexpected\t= %i\n\tnextframetosend\t= %i\n\tframeexpected\t= %i\t link = %d\t other address=%d\n", i, conn[i].ackexpected, conn[i].nextframetosend, conn[i].frameexpected, conn[i].link, conn[i].otheraddr);
    }
}
